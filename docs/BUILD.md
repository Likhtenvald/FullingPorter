...
The second command takes `artifacts\FullingPorter.dll`, checks that its assembly version matches the plugin, project and manifest, verifies the 256×256 icon, and creates `dist\FullingPorter-0.1.1-thunderstore.zip`. The archive contains only `manifest.json`, `icon.png`, `README.md`, `CHANGELOG.md` and `FullingPorter.dll` at its root. Dependencies are declared in the manifest and are not bundled. The script refuses to overwrite an archive with the same version.
